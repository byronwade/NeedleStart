...
6. CLI inspection: `lumina routes --json`, `lumina inspect --json`, `lumina inspect why`, minimal `lumina map affected --json`, `lumina bench --list --json`, `lumina bench <name> --json`, and `lumina bench route-discovery --json --run` are implemented for app, route, direct-import route-impact, benchmark-skeleton evidence, and route-discovery local raw metadata.
...
- Root layout.
- Nested layouts.
- Dynamic and catch-all page route params in the Vite dev server.
- Search params in the Vite dev server.
- Error page.
- Not found page.
