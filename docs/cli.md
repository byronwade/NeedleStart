...
| `lumina bench` | Run planned benchmark fixtures and emit evidence metadata. | Implemented for `--list --json`, `<name> --json` benchmark skeleton status, `route-discovery --json --run`, and `manifest-size --json --run`; persisted raw result files and public comparisons remain planned | Yes |
...
| `lumina bench <name>` | Implemented for reporting one benchmark skeleton status as compact JSON with `--json`; `route-discovery --json --run` and `manifest-size --json --run` also run local benchmarks and return raw metadata in the JSON response. |
