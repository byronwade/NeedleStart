...
Lumina is in Phase 1: monorepo scaffold with route discovery, explicit static/SSR render-mode extraction, generated route/render/map artifacts, direct local import edges, route, inspect, and minimal map affected CLI paths, a minimal Vite dev-server path with dynamic and catch-all page route params, search params, and route-specific dev hydration bundles, browser-verified interactive dev and production root-route hydration, static build/start output with route-specific production client bundles, and the early benchmark/status skeleton implemented.
...
- `@lumina/vite-plugin` minimal dev-server and static-build integration that writes route/render/map artifacts, serves static, dynamic, and catch-all page routes with React SSR through Vite middleware, passes route `params` and `searchParams` to page components, emits `.lumina/client/*.js` dev route hydration bundles, emits `dist/public/_lumina/client/*.js` production route hydration bundles, supports browser-verified interactive root-route hydration in dev and built output, exposes `virtual:lumina/routes`, and emits `.lumina/hmr-report.json` when app route files change.
...
- Component-level HMR and production dynamic route SSR.
