...
- `@lumina/cli` support for `routes <appPath> --json`, `inspect <appPath> --json`, `inspect <appPath> why <route>`, `map affected <appPath> <file> --json`, `bench --list --json`, `dev <appPath>`, `build <appPath>` with timed human phase output, and `start <appPath>`, exposed locally through `bun run lumina -- ...`.
