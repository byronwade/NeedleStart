...
- Early benchmark/status skeletons under `benchmarks/` and stable fixture placeholders under `fixtures/apps/`, with local `route-discovery` execution against `fixtures/apps/tiny-static`, remaining surfaces reporting `not implemented`, and no synthetic timing results.
