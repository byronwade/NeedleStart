...
- `@lumina/vite-plugin` minimal dev-server integration that writes route/render/map artifacts, serves page routes with React SSR through Vite middleware, emits `.lumina/client/*.js` route hydration bundles, supports a browser-verified interactive root-route hydration proof, exposes `virtual:lumina/routes`, and emits `.lumina/hmr-report.json` when app route files change.
