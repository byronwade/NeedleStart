...
Lumina is in Phase 1: monorepo scaffold with route discovery, explicit static/SSR render-mode extraction, generated route/render/map artifacts, direct local import edges, route, inspect, and minimal map affected CLI paths, a minimal Vite dev-server path with timed human startup output, dynamic and catch-all page route params, search params, not-found/error components, and route-specific dev hydration bundles, browser-verified interactive dev and production root-route hydration, static build/start output with timed human build phases and route-specific production client bundles, the early benchmark/status skeleton, and local route-discovery, manifest-size, graph-query, and adapter-dispatch benchmark execution implemented.
...
- Early benchmark/status skeletons under `benchmarks/` and stable fixture placeholders under `fixtures/apps/`, with local `route-discovery` execution against `fixtures/apps/tiny-static`, local `manifest-size` execution against deterministic generated source from `fixtures/apps/medium-100-routes`, local `graph-query` execution against deterministic generated source from `fixtures/apps/large-1000-routes`, local `adapter-dispatch` execution against built `fixtures/apps/tiny-static` output through `@lumina/adapter-bun`, and no synthetic timing results.
...
- Persisted or reviewed measured benchmark result files.
