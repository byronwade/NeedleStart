...
- `@lumina/vite-plugin` minimal dev-server and static-build integration that writes route/render/map artifacts, serves page routes with React SSR through Vite middleware, emits `.lumina/client/*.js` dev route hydration bundles, emits `dist/public/_lumina/client/*.js` production route hydration bundles, supports browser-verified interactive root-route hydration in dev and built output, exposes `virtual:lumina/routes`, and emits `.lumina/hmr-report.json` when app route files change.
...
- Component-level HMR and dynamic route params in the dev server.
