...
- `@lumina/cli` support for `routes <appPath> --json`, `inspect <appPath> --json`, `inspect <appPath> why <route>`, `map affected <appPath> <file> --json`, `bench --list --json`, `dev <appPath>` with timed human startup output, `build <appPath>` with timed human phase output, and `start <appPath>`, exposed locally through `bun run lumina -- ...`.
...
- Scaffolded `apps/www` marketing app source with shadcn-style UI primitives, Tailwind v4/shadcn configuration files, light/dark theme tokens, a persisted theme toggle, a static public-docs route scaffold for the docs home plus start, concepts, guide, reference, deployment, and community pages, an SSR `/docs/*` public-docs viewer backed by metadata and bundled Markdown body snapshots for the current `docs/public/` page set, verified starter example under `examples/basic/`, runnable content example under `examples/blog-seo/`, and scaffolded examples under `examples/multi-app-workspace/`, `examples/large-100-routes/`, and `examples/large-1000-routes/`, with route-discovery and inspect fixture evidence.
...
- Component-level HMR, production dynamic route SSR, and production not-found/error rendering.
