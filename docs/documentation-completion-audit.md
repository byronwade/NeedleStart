...
This section maps the current active goal to repository evidence. It is a documentation-system completion audit only; framework implementation remains Phase 1 scaffold.
...
| Features are consistent across docs and do not contradict current implementation status. | Root and public docs describe Phase 1 scaffold as Bun workspace, package placeholders, shared core types, CI, and enforcement scripts. They keep route discovery, rendering, CLI behavior, runtime adapter behavior, Needle Map generation, MCP tools, and safe edits marked planned. `bun run docs:check` rejects known stale pre-scaffold and unimplemented-behavior wording. | Covered for current docs system |
| CLI and example references stay honest before implementation exists. | `docs/cli.md`, `docs/public/reference/cli.md`, `docs/examples-contract.md`, `docs/examples-catalog.md`, and `docs/public/reference/examples.md` mark command behavior and examples as planned. `scripts/check-docs.ts` rejects an ambiguous present-tense `JSON output required?` CLI heading while CLI behavior is unimplemented. | Covered |
...
| Docs are ready to guide prototype build work. | `docs/product-build-readiness.md`, `docs/phase-1-build-plan.md`, `docs/task-backlog.md`, `docs/package-map.md`, `docs/prototype-acceptance.md`, `docs/roadmap.md`, and feature contracts define the next Phase 1A path, package boundaries, gates, acceptance criteria, risks, and evidence requirements without claiming behavior exists. | Covered for documentation readiness |
