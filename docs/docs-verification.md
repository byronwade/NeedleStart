...
Automated coverage in `bun run docs:check` also requires `docs/cli-json-contract.md` and `docs/public/reference/cli.md` to list every JSON automation command: implemented `lumina routes --json`, `lumina inspect --json`, `lumina map affected --json`, `lumina build --json`, `lumina bench --list --json`, and `lumina bench <name> --json`, plus planned `lumina build --affected --json`, `lumina check --json`, `lumina check --affected --json`, `lumina test --affected --json`, `lumina seo --json`, broader `lumina map --json`, `lumina workspace graph --json`, `lumina workspace apps --json`, `lumina workspace explain <file> --json`, `lumina agent context --json`, `lumina edit --json`, `lumina migrate --json`, and measured `lumina bench --json`. Public CLI docs must distinguish implemented routes, inspect, build, benchmark list/detail status, and minimal map affected commands from planned command surfaces.
