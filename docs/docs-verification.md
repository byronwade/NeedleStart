...
The current implementation path must stay aligned across README, project status, roadmap, public roadmap, product-readiness, and backlog docs: Phase 1A shared core model hardening is implemented, the first `@lumina/compiler` route-discovery package API exists, the early benchmark/status skeleton exists with `not implemented` status, static `@lumina/adapter-bun` serving exists for built HTML, and the next work connects client hydration plus production hardening. Current-status and verification docs must distinguish static `@lumina/adapter-bun` serving from planned SSR/API runtime adapter behavior.
...
Automated coverage in `bun run docs:check` also requires `docs/cli-json-contract.md` and `docs/public/reference/cli.md` to list every JSON automation command: implemented `lumina routes --json`, `lumina inspect --json`, `lumina map affected --json`, and `lumina build --json`, plus planned `lumina build --affected --json`, `lumina check --json`, `lumina check --affected --json`, `lumina test --affected --json`, `lumina seo --json`, broader `lumina map --json`, `lumina workspace graph --json`, `lumina workspace apps --json`, `lumina workspace explain <file> --json`, `lumina agent context --json`, `lumina edit --json`, `lumina migrate --json`, and `lumina bench --json`. Public CLI docs must distinguish implemented routes, inspect, build, and minimal map affected commands from planned command surfaces.

Automated coverage also requires the internal Lumina Map contract and public Lumina Map guide to list the map command surfaces: implemented `lumina map affected <appPath> <file> --json`, plus planned `lumina map`, `lumina map --json`, `lumina map file`, `lumina map route`, and `lumina map explain`.
...
Automated coverage in `bun run docs:check` also requires `docs/documentation-completion-audit.md` to use documentation coverage labels and documentation-scoped audit headings, not broad completion status labels. Completion-audit rows must distinguish covered documentation from implemented framework behavior, launch readiness, package readiness, and benchmark evidence.
