...
Lumina is intentionally competing on difficult axes: semantic graph extraction, agent-safe workflows, safe automated edits, Bun-first adapter paths, and adoption against mature React frameworks.
...
- Adoption must lead with the agent and Lumina Map workflow, not generic framework parity.
...
- `lumina migrate from-next` becomes an early adoption prototype.
...
- Treat Lumina Map as a devtools feature instead of a core framework primitive.
