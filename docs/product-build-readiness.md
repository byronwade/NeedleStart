...
- An agent enforcement matrix that maps docs, structure, performance, typecheck, and test gates to the rules they currently enforce.
...
This documentation foundation is present for the Phase 1 scaffold when `bun run check` passes. Phase 1A shared core model hardening is implemented in `@lumina/core`, the first route-discovery package API is implemented in `@lumina/compiler`, the minimal dev-server and static-build APIs are implemented in `@lumina/vite-plugin`, the first dev and production hydration bundle output and browser-verified interactive hydration proof exist, direct local imported component affected-route reports exist, the first static serving path is implemented in `@lumina/adapter-bun`, and the early benchmark/status skeleton exists with route-discovery and manifest-size local run support plus remaining `not implemented` status surfaces, specifically graph-query and adapter-dispatch. The current follow-up is keeping the readiness checklist aligned with implementation issues, PR descriptions, broader component-level browser HMR, and production hardening after the dev and production hydration proof.
