...
| `@lumina/vite-plugin` | Implemented minimal Vite dev-server integration for artifact generation, SSR page serving, route-specific dev hydration bundles, `virtual:lumina/routes`, route-file update reports, and static build-time HTML rendering; planned production hydration bundles, component-level HMR, app-scoped invalidation, and server entry wiring. |
