...
| Client payload | Enforce public route JS, CSS, hydration, and LCP asset budgets. | INP and LCP regressions often come from excess client work. | `.needle/perf.report.json`, browser evidence, budget diagnostics. |
...
| Caching | Explicit cache plans, tags, headers, and revalidation metadata. | Hidden caching causes correctness and security failures. | Cache manifest snapshots and adapter header tests. |
