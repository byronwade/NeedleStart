...
Lumina is preparing an MVP Alpha focused on route discovery, explicit render modes, the first Lumina Map output, CLI inspection, and the first dev/build/start path. The current repository has route discovery in `@lumina/compiler`, generated `.lumina/routes.json`, `.lumina/render-manifest.json`, and `.lumina/map.json` output, the first route and inspect CLI paths, a minimal `lumina map affected` query, `lumina bench --list --json`, `lumina bench <name> --json` skeleton status output, local `lumina bench route-discovery --json --run`, `lumina bench manifest-size --json --run`, and `lumina bench graph-query --json --run` execution with raw metadata in the JSON response, minimal `lumina dev` with timed human startup output, static `lumina build` with timed human phase output, and static `lumina start` paths; broader MVP commands below are target behavior until implementation and fixture evidence exist.

Current implemented scope is the Bun workspace scaffold, package placeholders, contract-backed shared core model types, `@lumina/react` `staticPage()` and `ssr()` render helpers, `@lumina/compiler` route discovery and explicit static/SSR render-mode extraction, compact `.lumina/routes.json`, `.lumina/render-manifest.json`, and `.lumina/map.json` generation with direct local import edges, MVP Bun `lumina.config.ts` parsing and diagnostics, `bun run lumina -- routes <appPath> --json`, `bun run lumina -- inspect <appPath> --json`, `bun run lumina -- inspect <appPath> why <route>`, `bun run lumina -- map affected <appPath> <file> --json`, `bun run lumina -- bench --list --json`, `bun run lumina -- bench <name> --json`, `bun run lumina -- bench route-discovery --json --run`, `bun run lumina -- bench manifest-size --json --run`, `bun run lumina -- bench graph-query --json --run`, `bun run lumina -- dev <appPath>` with timed human startup output, minimal Vite SSR dev serving for static, dynamic, and catch-all page routes with page `params` and `searchParams`, app-level and route-level `not-found.tsx` / `error.tsx` rendering in dev, first dev client hydration bundles under `.lumina/client/*.js`, browser-verified interactive dev hydration for the `apps/www` root route, `virtual:lumina/routes`, route-file artifact regeneration and direct local imported component affected-route reporting with `.lumina/hmr-report.json`, `bun run lumina -- build <appPath>` static HTML output with timed human phase output and route-specific production client bundles under `dist/public/_lumina/client/*.js`, `.lumina/generated/server-entry.ts`, `bun run lumina -- start <appPath>` static built-output serving through `@lumina/adapter-bun` with no source route files required on the request path, tested static HTML and client asset cache headers, stable 404s, sanitized malformed-path responses, and public `.txt` / `.json` asset content types, browser-verified interactive production hydration for the built `apps/www` root route, `.lumina/build-trace.json`, `.lumina/perf.report.json`, deployment manifest copies under `dist/`, scaffolded `apps/www` and example fixtures, a shadcn-style `apps/www` public UI layer with Tailwind v4/shadcn configuration files, light/dark theme tokens, a persisted theme toggle, a theme-aware graph relationship hero, a static public-docs route scaffold with a docs home plus start, concepts, guide, reference, deployment, and community pages, an SSR `/docs/search` route backed by a bundled public-docs index, an SSR `/docs/*` public-docs viewer backed by metadata and bundled Markdown body snapshots for the current `docs/public/` page set, and deterministic `apps/www` build preview artifacts at `dist/public/docs-index.json`, `dist/public/docs-navigation.json`, `dist/public/llms.txt`, and `dist/public/llms-full.txt`, early benchmark/status skeletons with route-discovery, manifest-size, and graph-query local execution support plus the remaining `not implemented` adapter-dispatch benchmark surface, CI, and verification scripts. Persisted benchmark result files, reviewed public benchmark evidence, frontmatter parsing, generated per-document static routes, broader machine-readable docs output beyond the `apps/www` preview artifacts, API production behavior, broader component-level browser HMR, broader Lumina Map query modes, MCP tools, and safe edits remain planned.
...
The verified starter example path is `examples/basic/`. Runnable fixture paths are `examples/blog-seo/`, `examples/large-100-routes/`, and `examples/large-1000-routes/`; the large-route fixtures are runnable only for deterministic source generation, route discovery, generated route/render/map artifacts, CLI JSON, and manifest-size readiness, not dev/build/start app behavior. The scaffolded workspace example path is `examples/multi-app-workspace/`. Planned future example paths include `examples/api-route/`, `examples/hot-api/`, `examples/static-export/`, `examples/adapter-node/`, `examples/dashboard-client/`, `examples/ecommerce/`, `examples/agent-demo/`, `examples/docs-site/`, and `playgrounds/large-app-fixture/`. These planned paths are target inventory only until the directories, commands, tests, generated artifacts, and status labels exist.
...
9. Minimal Vite dev SSR integration with `virtual:lumina/routes`, route-file update reports, and direct local imported component affected-route reports.
10. React hydration and broader component-level browser HMR.
...
The first dev-server implementation exists for minimal Vite SSR page rendering, dynamic and catch-all page route params, search params, app-level and route-level not-found/error components, route-specific dev client hydration bundles, browser-verified interactive root-route hydration, `virtual:lumina/routes`, route-file update reports, and direct local imported component affected-route reports. Static build output, route-specific production client bundles, browser-verified production root-route hydration, and hardened static built-output serving through `@lumina/adapter-bun` exist for routes that can be rendered at build time. Local raw benchmark metadata exists for route discovery, manifest size, and graph-query runs. Persisted reviewed benchmark results, SSR/API production behavior, broader component-level browser HMR, MCP tools, and safe edits remain planned. The next implementation stage is completing component-level HMR and production hardening after the dev and production hydration proof.
